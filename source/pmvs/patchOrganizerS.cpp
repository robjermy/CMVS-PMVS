...
void PMVS3::CPatchOrganizerS::image2index(Patch::CPatch& patch) {
  ...
  for (int i = 0; i < (int)patch._images.size(); ++i) {
    const int index = _fm.PhotoSets().image2index(patch._images[i]);
    if (index != -1) {
...
void PMVS3::CPatchOrganizerS::index2image(Patch::CPatch& patch) {
  for (int i = 0; i < (int)patch._images.size(); ++i) {
    patch._images[i] = _fm.PhotoSets()._images[patch._images[i]];
  }

  for (int i = 0; i < (int)patch._vimages.size(); ++i) {
    patch._vimages[i] = _fm.PhotoSets()._images[patch._vimages[i]];
  }
}
...
void PMVS3::CPatchOrganizerS::init(void) {
  ...
  for (int index = 0; index < _fm.NumImages(); ++index) {
    const int gwidth = (_fm.PhotoSets().getWidth(index, _fm.Level()) + _fm.CSize() - 1) / _fm.CSize();
    const int gheight = (_fm.PhotoSets().getHeight(index, _fm.Level()) + _fm.CSize() - 1) / _fm.CSize();
...
void PMVS3::CPatchOrganizerS::readPatches(void) {
  ...
  for (int i = 0; i < _fm.NumTargetImages(); ++i) {
    const int image = _fm.Images()[i];
    char buffer[1024];
    sprintf(buffer, "%smodels/%08d.patc%d", _fm.Prefix().c_str(), image, _fm.Level());
    std::ifstream ifstr;
    ifstr.open(buffer);
...
void PMVS3::CPatchOrganizerS::updateDepthMaps(Patch::PPatch& ppatch) {
  for (int image = 0; image < _fm.NumTargetImages(); ++image) {
    const Vec3f icoord = _fm.PhotoSets().project(image, ppatch->_coord, _fm.Level());
    ...
    const float depth = _fm.PhotoSets()._photos[image].OpticalAxis() * ppatch->_coord;
    ...
    for (int j = 0; j < 2; ++j) {
      for (int i = 0; i < 2; ++i) {
        ...
        const int index = ys[j] * _gwidths[image] + xs[i];
          ...
          const float dtmp = _fm.PhotoSets()._photos[image].OpticalAxis() * _dpgrids[image][index]->_coord;
...
void PMVS3::CPatchOrganizerS::setGridsImages(Patch::CPatch& patch, const std::vector<int>& images) const {
  ...
  while (bimage != eimage) {
    ...
    const int ix = ((int)floor(icoord[0] + 0.5f)) / _fm.CSize();
    const int iy = ((int)floor(icoord[1] + 0.5f)) / _fm.CSize();
...
void PMVS3::CPatchOrganizerS::setGrids(Patch::CPatch& patch) const {
  ...
  for (int i = 0; i < (int)patch._images.size(); ++i) {
    ...
    Vec3f icoord = _fm.PhotoSets().project(image, patch._coord, _fm.Level());
    const int ix = ((int)floor(icoord[0] + 0.5f)) / _fm.CSize();
    const int iy = ((int)floor(icoord[1] + 0.5f)) / _fm.CSize();
...
void PMVS3::CPatchOrganizerS::setVImagesVGrids(Patch::CPatch& patch) {
  ...
  for (int image = 0; image < _fm.NumTargetImages(); ++image) {
    ...
    int ix, iy;
    if (isVisible0(patch, image, ix, iy, _fm.NeighborThreshold0(), 1) == 0) continue;
    if (_fm.PhotoSets().getEdge(patch._coord, image, _fm.Level()) == 0) continue;
...
int PMVS3::CPatchOrganizerS::isVisible0(const Patch::CPatch& patch, const int image, int& ix, int& iy, const float strict, const int lock) {
  const Vec3f icoord = _fm.PhotoSets().project(image, patch._coord, _fm.Level());
  ix = ((int)floor(icoord[0] + 0.5f)) / _fm.CSize();
  iy = ((int)floor(icoord[1] + 0.5f)) / _fm.CSize();
...
int PMVS3::CPatchOrganizerS::isVisible(const Patch::CPatch& patch, const int image, const int& ix, const int& iy, const float strict, const int lock) {
  ...
  Vec4f ray = patch._coord - _fm.PhotoSets()._photos[image].OpticalCenter();
  unitize(ray);
  const float diff = ray * (patch._coord - dppatch->_coord);
  const float factor = std::min(2.0, 2.0 + ray * patch._normal);

  if (diff < _fm.Optimizer().getUnit(image, patch._coord) * _fm.CSize() * strict * factor) {
    return 1;
  } else {
    return 0;
  }
...
void PMVS3::CPatchOrganizerS::findNeighbors(const Patch::CPatch& patch, std::vector<Patch::PPatch>& neighbors, const int lock, const float scale, const int margin, const int skipvis) {
  const float radius = 1.5 * margin * _fm.Expand().computeRadius(patch);
  ...
  while (bimage != eimage) {
    ...
    for (int j = -margin; j <= margin; ++j) {
      const int ytmp = iy + j;
      if (ytmp < 0 || _fm.PatchOrganizer()._gheights[image] <= ytmp) continue;

      for (int i = -margin; i <= margin; ++i) {
        const int xtmp = ix + i;

        if (xtmp < 0 || _fm.PatchOrganizer()._gwidths[image] <= xtmp) continue;

        const int index = ytmp * _fm.PatchOrganizer()._gwidths[image] + xtmp;
        std::vector<Patch::PPatch>::const_iterator bpatch = _fm.PatchOrganizer()._pgrids[image][index].begin();
        std::vector<Patch::PPatch>::const_iterator epatch = _fm.PatchOrganizer()._pgrids[image][index].end();
        while (bpatch != epatch) {
          if (_fm.isNeighborRadius(patch, **bpatch, unit, _fm.NeighborThreshold0() * scale, radius))
            neighbors.push_back(*bpatch);
          ++bpatch;
        }
        bpatch = _fm.PatchOrganizer()._vpgrids[image][index].begin();
        epatch = _fm.PatchOrganizer()._vpgrids[image][index].end();
        while (bpatch != epatch) {
          if (_fm.isNeighborRadius(patch, **bpatch, unit, _fm.NeighborThreshold0() * scale, radius)) {
            neighbors.push_back(*bpatch);
          }
...
void PMVS3::CPatchOrganizerS::setScales(Patch::CPatch& patch) const {
  const float unit = _fm.Optimizer().getUnit(patch._images[0], patch._coord);
  const float unit2 = 2.0f * unit;
  Vec4f ray = patch._coord - _fm.PhotoSets()._photos[patch._images[0]].OpticalCenter();
  unitize(ray);
  ...
  for (int i = 1; i < inum; ++i) {
    Vec3f diff = _fm.PhotoSets().project(patch._images[i], patch._coord, _fm.Level()) - _fm.PhotoSets().project(patch._images[i], patch._coord - ray * unit2, _fm.Level());
  ...
  patch._ascale = atan(patch._dscale / (unit * _fm.WSize() / 2.0f));
}
  ...
  while (bpatch != bend) {
    ...
    if (mode == 0) {
      ...
      for (int i = 0; i < (int)(*bpatch)->_images.size(); ++i) {
        ...
        colorf += _fm.PhotoSets().getColor((*bpatch)->_coord, image, _fm.Level());
        denom++;
      }
      ...
      while (bimage != eimage) {
        const int index = *bimage;
        Vec4f ray = _fm.PhotoSets()._photos[index].OpticalCenter() - (*bpatch)->_coord;
        ray[3] = 0.0f;
        unitize(ray);
