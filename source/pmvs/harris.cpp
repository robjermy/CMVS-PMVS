...
void PMVS3::CHarris::setDerivatives(void) {
  // set _dIdx, _dIdy
  preprocess();
...
void PMVS3::CHarris::preprocess2(void) {
  _dIdxdIdx.clear();
  _dIdydIdy.clear();
  _dIdxdIdy.clear();
  ...
  {
    std::vector<std::vector<Vec3f> >().swap(_dIdx);
    std::vector<std::vector<Vec3f> >().swap(_dIdy);
  }
...
void PMVS3::CHarris::preprocess(void) {
...
void PMVS3::CHarris::setResponse(void) {
  _response.clear();
  ...
  std::vector<std::vector<float> > vvftmp = _response;
...
void PMVS3::CHarris::run(
  ...
  for (int y = margin; y < _height - margin; ++y) {
    for (int x = margin; x < _width - margin; ++x) {
      if (_response[y][x] == 0.0) continue;

      const int x0 = std::min(x / gridsize, w - 1);
      const int y0 = std::min(y / gridsize, h - 1);

      if ((int)resultgrids[y0][x0].size() < maxPointsGrid || resultgrids[y0][x0].begin()->_response < _response[y][x]) {
        CPoint p;
