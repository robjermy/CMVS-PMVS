...
#include "image/photo.hpp"

Image::CPhoto::CPhoto() {}
Image::CPhoto::~CPhoto() {}

void Image::CPhoto::init(const std::string name, const std::string mname, const std::string cname, const int maxLevel) {
  CImage::init(name, mname, maxLevel);
  CCamera::init(cname, maxLevel);
}

void Image::CPhoto::init(const std::string name, const std::string mname, const std::string ename, const std::string cname, const int maxLevel) {
  CImage::init(name, mname, ename, maxLevel);
  CCamera::init(cname, maxLevel);
}

float Image::CPhoto::ssd(const std::vector<Vec3f>& tex0, const std::vector<Vec3f>& tex1) {
  float ans = 0.0f;
...
float Image::CPhoto::idot(const std::vector<Vec3f>& tex0, const std::vector<Vec3f>& tex1) {
  if (tex0.empty() || tex1.empty()) {
    std::cerr << "Error in idot. Empty textures" << std::endl;
    exit (1);
  }
...
void Image::CPhoto::idotC(const std::vector<Vec3f>& tex0, const std::vector<Vec3f>& tex1, double* idc) {
  if (tex0.empty() || tex1.empty()) {
    std::cerr << "Error in idotC. Empty textures" << std::endl;
    exit (1);
  }
...
void Image::CPhoto::normalize(std::vector<Vec3f>& tex) {
  // normalize average
  Vec3f ave;
...
void Image::CPhoto::grabTex(const int level, const Vec2f& icoord, const Vec2f& xaxis, const Vec2f& yaxis, const int size, std::vector<Vec3f>& tex, const int normalizef) const{
  ...
  tex.clear();
  if (minx < 0 || getWidth(level) - 1 <= maxx || miny < 0 || getHeight(level) - 1 <= maxy) return;
...
void Image::CPhoto::grabTex(const int level, const Vec4f& coord, const Vec4f& pxaxis, const Vec4f& pyaxis, const Vec4f& pzaxis, const int size, std::vector<Vec3f>& tex, float& weight, const int normalizef) const {
  ...
  Vec4f ray = OpticalCenter() - coord;
  unitize(ray);
  weight = std::max(0.0f, pzaxis * ray);
}
