#include <iostream>
#include "cmvs/graclus.hpp"

CMVS::CGraclus::CGraclus() {}
...
**************************************************************************/
void CMVS::CGraclus::initGraph(GraphType& graph) {
...
int CMVS::CGraclus::mylog2(int a) {
  int i;
  for (i = 1 ; a > 1; i++, a = a>>1);
  return i-1;
}

void CMVS::CGraclus::runSub(
  GraphType& graph,
  int nparts,
  int cutType,
  int wgtflag,
  ...
  const int levels = amax((graph.nvtxs)/(40*mylog2(nparts)), 20*(nparts));
