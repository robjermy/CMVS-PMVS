...
public:
  ...
  inline TVec4<T> col(int i) const {
    return TVec4<T>(_row[0][i],_row[1][i],_row[2][i],_row[3][i]);
  }
...
inline bool TMat4<T>::operator==(const TMat4<T>& m) const {
  return _row[0] == m._row[0] && _row[1] == m._row[1] && _row[2] == m._row[2] && _row[3] == m._row[3];
}
...
inline TVec3<T> operator*(const TMat4<T>& m, const TVec3<T>& v) {
  ...
  if(w == 0.0) {
    return TVec3<T>(m[0]*u, m[1]*u, m[2]*u);
  }

  return TVec3<T>(m[0]*u/w, m[1]*u/w, m[2]*u/w);
}
...
//
template<typename T>
inline T det(const TMat4<T>& m) {
  return m[0] * cross(m[1], m[2], m[3]);
}

template<typename T>
inline T trace(const TMat4<T>& m) {
...
TMat4<T> viewport_matrix(T w, T h) {
  return scaling_matrix(TVec3<T>(w/2.0, -h/2.0, 1)) * translation_matrix(TVec3<T>(1, -1, 0));
}
...
T invert(TMat4<T>& B, const TMat4<T>& m) {
  ...
  /*---------- forward elimination ----------*/
  B = TMat4<T>::I(); /* put identity matrix in B */
