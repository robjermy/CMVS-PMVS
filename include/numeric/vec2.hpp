...
// Method definitions
//
template<typename T> inline TVec2<T>& TVec2<T>::operator+=(const TVec2<T>& v) {
  _elt[0] += v[0];
  _elt[1] += v[1];
  return *this;
}
...
template<typename T> inline TVec2<T>& TVec2<T>::operator*=(T s) {
  _elt[0] *= s;
  _elt[1] *= s;
  return *this;
}
...
template<typename T> inline bool TVec2<T>::operator==(const TVec2<T>& v) const{
  return _elt[0] == v._elt[0] && _elt[1] == v._elt[1];
};
...
template<typename T> inline TVec2<T> operator-(const TVec2<T> &v) {
  return TVec2<T>(
    -v[0],
    -v[1]
  );
...
template<typename T, typename N> inline TVec2<T> operator*(N s, const TVec2<T> &v) {
  return TVec2<T>(
    v[0]*s,
    v[1]*s
  );
}
template<typename T, typename N> inline TVec2<T> operator*(const TVec2<T> &v, N s) {
  return s*v;
}
...
template<typename T> inline T operator*(const TVec2<T> &u, const TVec2<T>& v) {
  return u[0]*v[0] + u[1]*v[1];
}

template<typename T> inline TVec2<T> perp(const TVec2<T> &v) {
  return TVec2<T>(
    v[1],
    -v[0]
  );
...
//----------------------------------------------------------------------
template<typename T> inline T norm2(const TVec2<T>& v) {
  return v*v;
}

template<typename T> inline T norm(const TVec2<T>& v) {
  return sqrt(norm2(v));
}

template<typename T> inline void unitize(TVec2<T>& v) {
  T l = norm2(v);
  if(l != 1.0 && l != 0.0) {
    v /= sqrt(l);
  }
...
template<typename T>
int isOverlapUnsorted(const TVec2<T>& lhs, const TVec2<T>& rhs, TVec2<T>& intersect) {
...
template<typename T>
int isOverlap(const TVec2<T>& lhs, const TVec2<T>& rhs, TVec2<T>& intersect) {
...
template<typename T>
T isIntersect(const TVec2<T>& p0, const TVec2<T>& p1, const TVec2<T>& q0, const TVec2<T>& q1) {
